...
A large international community of developers worked hard to prepare a new release of the Pharo platform like the implementation of a new UI framework that accepts different back-ends (currently our own, Morphic, plus Gtk3), the use of it to re-implement in an revolutionary way some of the tools of the environment (playground, inspector, debugger, spotter), incorporating the new concept of "object centric debugging" and a big set of changes in the virtual machine level, from a real AIO event-driven VM to support for ARM64 and including the new Apple M1! 
...
- first fully working ARM-based JIT for Windows, Raspberian and M1. 
